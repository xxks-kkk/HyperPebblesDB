...
static Status
IOError(const std::string &context, int err_number)
{
...
private:
    ...
    PosixSequentialFile &
    operator=(const PosixSequentialFile &);
...
private:
    ...
    operator=(const PosixMmapReadableFile &);
    std::string filename_;
...
private:
    ...
    PosixWritableFile &
    operator=(const PosixWritableFile &);
    std::string filename_;
    FILE *file_;
...
private:
    ...
    bool
    GrowViaTruncate(uint64_t block)
    {
        mtx_.Lock();
...
public:
    ...
    virtual Status
    WriteAt(uint64_t offset, const Slice &data)
    {
...
public:
    ...
    bool
    Insert(const std::string &fname)
    {
        MutexLock l(&mu_);
        return locked_files_.insert(fname).second;
    }
    void
    Remove(const std::string &fname)
    {
        MutexLock l(&mu_);
...
public:
    ...
    virtual Status
    CreateDir(const std::string &name)
    {
        Status result;
    ...
    virtual Status
    DeleteDir(const std::string &name)
    {
        Status result;
    ...
    virtual Status
    GetFileSize(const std::string &fname, uint64_t *size)
    {
        Status s;
        struct stat sbuf;
    ...
    virtual Status
    CopyFile(const std::string &src, const std::string &target)
    {
        Status result;
        int fd1 = -1;
        int fd2 = -1;
    ...
    virtual void
    Schedule(void (*function)(void *), void *arg);

    virtual void
    StartThread(void (*function)(void *arg), void *arg);

    virtual pthread_t
    StartThreadAndReturnThreadId(void (*function)(void *arg), void *arg);

    virtual void
    WaitForThread(unsigned long int th, void **return_status);

    virtual Status
    GetTestDirectory(std::string *result)
    {
...
PosixEnv::BGThread()
{
    while (true)
    {
        ...
        void
        (*function)(void *) = queue_.front().function;
...
void
PosixEnv::StartThread(void (*function)(void *arg), void *arg)
{
    pthread_t t;
...
pthread_t
PosixEnv::StartThreadAndReturnThreadId(void (*function)(void *arg), void *arg)
{
    pthread_t t;
    StartThreadState *state = new StartThreadState;
    state->user_function = function;
...
void
PosixEnv::WaitForThread(unsigned long int th, void **return_status)
{
...
static void
InitDefaultEnv() { default_env = new PosixEnv; }

void
EnvPosixTestHelper::SetReadOnlyFDLimit(int limit)
{
...
void
EnvPosixTestHelper::SetReadOnlyMMapLimit(int limit)
{
    assert(default_env == nullptr);
    mmap_limit = limit;
}
