...
static void
AppendWithSpace(std::string *str, Slice msg)
{
    if (msg.empty())
        return;
    if (!str->empty())
...
public:
    ...
    int
    split_file_names(const char *file, char file_names[20][100])
    {
        char delimiter = ',';
        int index = 0;
        int cur = 0;
    ...
    void
    parse_trace(const char *file, int tid)
    {
        int ret;
        char *buf;
        FILE *fp;
    ...
    Status
    perform_op(DB *db, struct trace_operation_t *op, int tid)
    {
        char keybuf[100];
        int keylen;
        Status status;
...
#define envstrinput(var) strcpy(var, getenv(#var))
    ...
    void
    ReliabilityCheck(ThreadState *thread)
    {
    ...
    void
    ReliabilityStart(ThreadState *thread)
    {
    ...
    Run()
    {
        ...
        while (benchmarks != nullptr)
        {
            ...
            void
            (Benchmark::*method)(ThreadState *) = nullptr;
            bool fresh_db = false;
            int num_threads = FLAGS_threads;
...
//        entries_per_batch_ = 1000;
            ...
            else if (name == Slice("emptyGuards"))
            {
                PrintStats("leveldb.emptyGuards");
            }
            else
            {
                if (name != Slice())
            ...
            if (fresh_db)
            {
                if (FLAGS_use_existing_db)
                {
                    fprintf(stdout, "%-12s : skipped (--use_existing_db is true)\n",
                            name.ToString().c_str());
                    method = nullptr;
                    // Uncommented the lines below if you don't want "fillseq" support when use --use_existing_db option,
                    // which is the default behavior in PebblesDB
//                delete db_;
//                db_ = nullptr;
//                Open();
                }
    ...
    int
    VerifyIteration(int print_every = 100000000)
    {
        int count = 0;
...
private:
    ...
    void
    AcquireLoad(ThreadState *thread)
    {
        int dummy;
    ...
    void
    SnappyCompress(ThreadState *thread)
    {
        RandomGenerator gen;
    ...
    void
    SnappyUncompress(ThreadState *thread)
    {
        RandomGenerator gen;
        Slice input = gen.Generate(Options().block_size);
        std::string compressed;
    ...
    void
    PrintDB(ThreadState *thread)
    {
        print_current_db_contents();
    }

    void
    CompactOnce(ThreadState *thread)
    {
    ...
    void
    ReduceActiveLevelsByOne(ThreadState *thread)
    {
    ...
    void
    CompactMemtable(ThreadState *thread)
    {
    ...
    void
    CompactAllLevels(ThreadState *thread)
    {
        print_current_db_contents();
    ...
    void
    WaitForStableState(ThreadState *thread)
    {

    }

    void
    WaitForStableStateSinglLevel(ThreadState *thread)
    {
        if (num_ != FLAGS_num)
        {
            char msg[100];
    ...
    void
    WriteSeq(ThreadState *thread)
    {
        DoWrite(thread, true);
    }

    void
    Reopen(ThreadState *thread)
    {
    ...
    void
    WriteRandom(ThreadState *thread)
    {
...
//#endif
    ...
    void
    ReadSequential(ThreadState *thread)
    {
        Iterator *iter = db_->NewIterator(ReadOptions());
        int i = 0;
        int64_t bytes = 0;
    ...
    void
    ReadReverse(ThreadState *thread)
    {
        Iterator *iter = db_->NewIterator(ReadOptions());
        int i = 0;
        int64_t bytes = 0;
    ...
    void
    ReadMissing(ThreadState *thread)
    {
        ReadOptions options;
        std::string value;
        for (int i = 0; i < reads_; i++)
    ...
    void
    DoDelete(ThreadState *thread, bool seq)
    {
        RandomGenerator gen;
        WriteBatch batch;
        Status s;
...
//#endif
    ...
    void
    DeleteSeq(ThreadState *thread)
    {
        DoDelete(thread, true);
    }

    void
    DeleteRandom(ThreadState *thread)
    {
        DoDelete(thread, false);
    }

    void
    SeekWhileWriting(ThreadState *thread)
    {
        if (thread->tid > 0)
        {
            SeekRandom(thread);
    ...
    void
    ReadWhileWriting(ThreadState *thread)
    {
        if (thread->tid > 0)
        {
            ReadRandom(thread);
    ...
    void
    Compact(ThreadState * /*thread*/)
    {
        db_->CompactRange(NULL, NULL);
    }

    void
    PrintStats(const char *key)
    {
    ...
    static void
    WriteToFile(void *arg, const char *buf, int n)
    {
