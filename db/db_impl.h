...
public:
    ...
    virtual Status
    Put(const WriteOptions &, const Slice &key, const Slice &value);
    virtual Status
    Delete(const WriteOptions &, const Slice &key);
    virtual Status
    ...
    Get(const ReadOptions &options,
        ...
        std::string *value);
    virtual Status
    GetCurrentVersionState(std::string *value);
    virtual Iterator *
    NewIterator(const ReadOptions &);
    virtual void
    GetReplayTimestamp(std::string *timestamp);
    virtual void
    AllowGarbageCollectBeforeTimestamp(const std::string &timestamp);
    virtual bool
    ValidateTimestamp(const std::string &timestamp);
    virtual int
    CompareTimestamps(const std::string &lhs, const std::string &rhs);
    virtual Status
    GetReplayIterator(const std::string &timestamp,
                      ReplayIterator **iter);
    virtual void
    ReleaseReplayIterator(ReplayIterator *iter);
    virtual const Snapshot *
    GetSnapshot();
    virtual void
    ...
    virtual void
    GetApproximateSizes(const Range *range, int n, uint64_t *sizes);
    virtual void
    CompactRange(const Slice *begin, const Slice *end);
    virtual Status
    LiveBackup(const Slice &name);
    virtual void
    PrintTimerAudit();
    virtual void
    ClearTimer();
    ...
    void
    TEST_CompactRange(unsigned level, const Slice *begin, const Slice *end);
    ...
    void
    TEST_ComapactFilesToSingleLevel();

    // Hacky function to forcefully fit all files to a single level inorder to do a comparison for seek benchmark
    void
    TEST_ReduceNumActiveLevelsByOne();

    // Record a sample of bytes read at the specified internal key.
    // Samples are taken approximately once every config::kReadBytesPeriod
    // bytes.
    void
    RecordReadSample(Slice key);
...
private:
    ...
    Iterator *
    NewInternalIterator(const ReadOptions &, uint64_t number,
    ...
    Status
    Recover(VersionEdit *edit) EXCLUSIVE_LOCKS_REQUIRED(mutex_);

    void
    MaybeIgnoreError(Status *s) const;
    ...
    Status
    WriteLevel0Table(MemTable *mem, VersionEdit *edit, Version *base, uint64_t *number)
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);

    Status
    WriteLevel0TableGuards(MemTable *mem, VersionEdit *edit, Version *base,
                           std::vector<uint64_t> &numbers, FileLevelFilterBuilder *file_level_filter_builder,
                           std::vector<std::string *> *file_level_filters)
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);

    Status
    SequenceWriteBegin(Writer *w, WriteBatch *updates)
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    void
    SequenceWriteEnd(Writer *w, WriteBatch *updates_with_guards, Status status)
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    ...
    CompactLevelThread();
    Status
    BackgroundCompaction() EXCLUSIVE_LOCKS_REQUIRED(mutex_);

    Status
    BackgroundCompactionGuards(FileLevelFilterBuilder *file_level_filter_builder) EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    Status
    BackgroundCompactionGuardsParallel(FileLevelFilterBuilder *file_level_filter_builder) EXCLUSIVE_LOCKS_REQUIRED(mutex_);

    void
    RecordBackgroundError(const Status &s);

    void
    CleanupCompaction(CompactionState *compact)
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    Status
    ...
    Status
    FinishCompactionOutputFile(CompactionState *compact,
                               Iterator *input,
                               FileLevelFilterBuilder *file_level_filter_builder,
                               std::vector<uint64_t> *file_numbers,
                               std::vector<std::string *> *file_level_filters);
    Status
    InstallCompactionResults(CompactionState *compact,
                             const int level_to_add_new_files,
                             std::vector<uint64_t> file_numbers,
                             std::vector<std::string *> file_level_filters)
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);
    ...
    bool internal_parallel_guard_compaction_;
    ...
    private:
        ...
        ManualCompaction &
        operator=(const ManualCompaction &);
    };
...
extern Options
SanitizeOptions(const std::string &db,
