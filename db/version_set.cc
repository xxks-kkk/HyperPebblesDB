...
static double
MaxBytesForLevel(unsigned level)
{
...
static double
MinBytesForLevel(unsigned level)
{
    assert(level < leveldb::config::kNumLevels);
...
static double
MaxBytesPerGuardForLevel(unsigned level)
{
    assert(level < leveldb::config::kNumLevels);
    static const double bytes[] = {64 * 1048576.0,
                                   128 * 1048576.0,
...
static uint64_t
MinFileSizeForLevel(unsigned level)
{
    assert(level < leveldb::config::kNumLevels);
...
static uint64_t
MaxFileSizeForLevel(unsigned level)
{
    assert(level < leveldb::config::kNumLevels);
...
static int
MaxFilesPerGuardForLevel(unsigned level)
{
    assert(level < leveldb::config::kNumLevels);
...
static uint64_t
MaxCompactionBytesForLevel(unsigned level)
{
    return MaxFileSizeForLevel(level) * 16;
}

static int64_t
TotalFileSize(const std::vector<FileMetaData *> &files)
{
    int64_t sum = 0;
...
struct SortBySmallestGuard
{
    ...
    bool
    operator()(GuardMetaData *g1, GuardMetaData *g2) const
    {
...
bool
guards_sorting_func(GuardMetaData *g1, GuardMetaData *g2)
{
    const InternalKeyComparator *internal_comparator;
...
int
FindGuard(const InternalKeyComparator &icmp,
...
static bool
AfterFile(const Comparator *ucmp,
...
static bool
BeforeFile(const Comparator *ucmp,
           const Slice *user_key, const FileMetaData *f)
{
...
static bool
AfterGuard(const Comparator *ucmp,
...
static bool
BeforeGuard(const Comparator *ucmp,
            const Slice *user_key, const GuardMetaData *g)
{
    return (user_key != NULL &&
        ucmp->Compare(*user_key, g->guard_key.user_key()) < 0);
...
private:
    ...
    operator=(const LevelFileNumIterator &);
...
public:
    ...
    {
        if (glist_->size() == 0)
        { // If there are no guards, setting index_ to -1
            index_ = -1;
        }
        else
        {
            ...
            if (index_ == 0)
            { // If the target is less than first guard key, setting index_ to -1 to point to sentinels
                ParsedInternalKey parsed_key;
...
private:
    ...
    LevelGuardNumIterator &
    operator=(const LevelGuardNumIterator &);
...
static Iterator *
GetGuardIteratorSeq(void *arg1, const void *arg2, void *arg3, unsigned level, const ReadOptions &options,
...
static Iterator *
GetGuardIterator(void *arg1, const void *arg2, void *arg3, unsigned level,
                 const ReadOptions &options,
                 const Slice &file_values)
{
    TableCache *table_cache = reinterpret_cast<TableCache *> (arg1);
                                  ...
                                  unsigned level, uint64_t num) const
{
    return NewTwoLevelIteratorGuards(
        new LevelGuardNumIterator(vset_->icmp_, &guards_[level], &sentinel_files_[level], &files_[level], num,
                                  vset_->timer),
...
void
Version::AddSomeIterators(const ReadOptions &options, uint64_t num,
                          std::vector<Iterator *> *iters)
{
...
void
Version::AddSomeIteratorsGuards(const ReadOptions &options, uint64_t num,
                                std::vector<Iterator *> *iters)
{
...
void
Version::ForEachOverlapping(Slice user_key, Slice internal_key,
                            void *arg,
...
Status
Version::Get(const ReadOptions &options,
             const LookupKey &k,
             std::string *value,
             GetStats *stats)
{
    ...
    for (unsigned level = 0; level < config::kNumLevels; level++)
    {
        ...
        if (num_guards == 0        // If there are no guards in the level, look at the sentinel files
            || (guard_index ==
                0  // If there are guards in the level and guard_index is 0, key can either be in sentinel or in the first(0-index) guard
                && num_guards > 0
        ...
        for (uint32_t i = 0; i < num_files; ++i)
        {
            ...
            switch (saver.state)
            {
                case kNotFound:break;      // Keep searching in other files
                case kFound:return Status::OK();
                case kDeleted:s = Status::NotFound(Slice());  // Use empty error message for speed
                    return s;
                case kCorrupt:s = Status::Corruption("corrupted key for ", user_key);
                    return s;
                default:break;
            }
        }
...
bool
Version::RecordReadSample(Slice internal_key)
{
    ...
    struct State
    {
        ...
        static bool
        Match(void *arg, unsigned level, FileMetaData *f)
        {
...
bool
Version::OverlapInLevel(unsigned level,
                        const Slice *smallest_user_key,
                        const Slice *largest_user_key)
{
...
bool
Version::OverlapInLevelGuards(unsigned level,
                              const Slice *smallest_user_key,
                              const Slice *largest_user_key)
{
...
void
Version::GetOverlappingInputsGuards(
    unsigned level,
    const InternalKey *begin,
    const InternalKey *end,
    std::vector<FileMetaData *> *inputs,
...
void
Version::AddDeltaGuardsToEdit(VersionEdit *edit, std::set<int> level_to_load_complete_guards)
{
    assert(edit != NULL);
    ...
    for (int level = 0; level < config::kNumLevels; level++)
    {
        if (level_to_load_complete_guards.count(level) > 0)
        {
            ...
            {
                int compare = vset_->icmp_.user_comparator()->Compare(complete_guards_[level][i]->guard_key.user_key(),
...
std::string
Version::EmptyGuardsInfo() const
{
    std::string r;
    std::vector<std::vector<size_t> > num_files_per_guard_per_level;
    std::vector<std::vector<size_t> > empty_guards_guards_per_level;
    for (unsigned level = 0; level < config::kNumLevels; level++)
    {
        r.append("Level ");
        AppendNumberTo(&r, level);
        ...
        for (size_t i = 0; i < guards.size(); i++)
        {
            ...
            for (size_t i = 0; i < g->files.size(); i++)
            {
                ...
                AppendNumberTo(&r, g->files[i]);
                r.push_back('(');
                AppendNumberTo(&r, g->file_metas[i]->file_size);
                r.push_back(')');
            }
            r.append("], ");
            if (g->files.size() == 0)
        ...
        r.append("\n");
        empty_guards_guards[1] += guards.size();
        empty_guards_guards_per_level.push_back(empty_guards_guards);
        num_files_per_guard_per_level.push_back(num_files_per_guard);
    }

    r.append("---- STATS ----\n");
    r.append("        mean, variance, std, std/mean, #empty guards, # guards\n");
    size_t totalSum = 0;
    size_t totalN = 0;
    size_t totalX_square = 0;
    size_t num_guards = 0;
    size_t num_empty_guards = 0;
    for (unsigned level = 0; level < num_files_per_guard_per_level.size(); level++)
    {
        r.append("level ");
        AppendNumberTo(&r, level);
        r.append(": ");
        ...
        AppendDoubleTo(&r, stddev);
        r.append(", ");
        double_t fracOfMean = stddev / mean;
        AppendDoubleTo(&r, fracOfMean);
        r.append(", ");
        size_t empty_guards_per_level = empty_guards_guards_per_level[level][0];
        AppendDoubleTo(&r, empty_guards_per_level);
        r.append(", ");
        size_t guards_per_level = empty_guards_guards_per_level[level][1];
        AppendDoubleTo(&r, guards_per_level);
        r.append("\n");

        totalSum += sum;
        totalN += n;
        totalX_square += x_square;
    ...
    r.append("avg files per guard: ");
    double_t totalMean = (double_t) totalSum / totalN;
    AppendDoubleTo(&r, totalMean);
    r.append("\n");
    ...
    r.append("Stddev on files per guard: ");
    double_t totalStddev = sqrt(totalVariance);
    AppendDoubleTo(&r, totalStddev);
    r.append("\n");

    r.append("Std / Mean: ");
    double_t totalFracOfMean = totalStddev / totalMean;
    AppendDoubleTo(&r, totalFracOfMean);
    r.append("\n");
    return r;
}
...
private:
    ...
    struct BySmallestKey
    {
        ...
        bool
        operator()(FileMetaData *f1, FileMetaData *f2) const
        {
    ...
    struct BySmallestInternalKey
    {
        ...
        bool
        operator()(InternalKey k1, InternalKey k2) const
        {
...
#pragma GCC diagnostic ignored "-Wunsafe-loop-optimizations"
    ...
    ~Builder()
    {
        for (unsigned level = 0; level < config::kNumLevels; level++)
        {
            ...
            for (GuardSet::const_iterator it = added_complete_guards->begin();
                 it != added_complete_guards->end(); it++)
            {
                g_to_unref.push_back(*it);
            }
            delete added_guards;
...
#pragma GCC diagnostic pop
    ...
    void
    SaveTo(Version *v, int mtc = 0, VersionEdit *edit = NULL)
    {
        int a;
        BySmallestKey cmp;
            ...
            if (base_complete_guards.size() > 0)
            {
                ...
                for (int i = start; i < base_complete_guards.size(); i++)
                {
                    if (vset_->icmp_.user_comparator()->Compare(base_complete_guards[i]->guard_key.user_key(),
                                                                base_complete_guards[i - 1]->guard_key.user_key()) < 0)
                    {
                        unsorted_start = i;
                        break;
                    }
                ...
                std::vector<GuardMetaData *>::const_iterator
                    base_end_complete_g = base_iter_complete_g + unsorted_start;
                GuardMetaData *last_inserted = NULL;
                ...
                {
                    for (std::vector<GuardMetaData *>::const_iterator bpos
                        = std::upper_bound(base_iter_complete_g, base_end_complete_g, *added_iter_complete_g,
                                           guard_cmp);
    ...
    bool
    IsFileAlreadyPresent(std::vector<uint64_t> files, uint64_t current_file_number)
    {
    ...
    void
    PopulateFilesToGuardsAndSentinels(Version *v, unsigned level)
    {
                    ...
                    else
                    {
                        if (!IsFileAlreadyPresent(guards->at(guard_no - 1)->files, current_file->number))
                        {
                            ...
                            else
                            {
                                ...
                                if (vset_->icmp_.Compare(current_file->smallest, guards->at(guard_no - 1)->smallest)
                                    < 0)
                                {
                                    guards->at(guard_no - 1)->smallest = current_file->smallest;
                                }
    ...
    void
    MaybeAddFile(Version *v, unsigned level, FileMetaData *f)
    {
    ...
    void
    MaybeAddGuard(Version *v, unsigned level, GuardMetaData *g, GuardMetaData **last_inserted)
    {
        if ((*last_inserted) != NULL && vset_->icmp_.user_comparator()->Compare(g->guard_key.user_key(),
                                                                                (*last_inserted)->guard_key.user_key())
            ==
                0)
        {
            return;
    ...
    void
    AddCompleteGuard(Version *v, unsigned level, GuardMetaData *g)
    {
        if (levels_[level].deleted_guards->count(g->guard_key) > 0)
        {
            // Guard is deleted: do nothing
    ...
    void
    MaybeAddCompleteGuard(Version *v, unsigned level, GuardMetaData *g, GuardMetaData **last_inserted)
    {
        if ((*last_inserted) != NULL && vset_->icmp_.user_comparator()->Compare(g->guard_key.user_key(),
                                                                                (*last_inserted)->guard_key.user_key())
            ==
...
void
VersionSet::PrintSeekThreadsStaticTimerAuditIndividual()
{
    table_cache_->PrintSeekThreadsStaticTimerAuditIndividual();
}

void
VersionSet::PrintSeekThreadsStaticTimerAuditCumulative()
{
    table_cache_->PrintSeekThreadsStaticTimerAuditCumulative();
}

void
VersionSet::AppendVersion(Version *v)
{
    // Make "v" current
    assert(v->refs_ == 0);
    assert(v != current_);
...
void
VersionSet::PopulateFileLevelBloomFilter()
{
#ifdef FILE_LEVEL_FILTER
...
void
VersionSet::PopulateBloomFilterForFile(FileMetaData *file, FileLevelFilterBuilder *file_level_filter_builder)
{
...
void
VersionSet::InitializeFileLevelBloomFilter()
{
#ifdef FILE_LEVEL_FILTER
    PopulateFileLevelBloomFilter();
#endif
...
void
VersionSet::InitializeTableCacheFileMetaData()
{
    Version *current = current_;
    current->Ref();
    for (int level = 0; level < config::kNumLevels; level++)
...
void
VersionSet::AddFileLevelBloomFilterInfo(uint64_t file_number, std::string *filter_string)
{
#ifdef FILE_LEVEL_FILTER
...
void
VersionSet::RemoveFileLevelBloomFilterInfo(uint64_t file_number)
{
#ifdef FILE_LEVEL_FILTER
...
void
VersionSet::RemoveFileMetaDataFromTableCache(uint64_t file_number)
{
    table_cache_->RemoveFileMetaDataMapForFile(file_number);
}

Status
VersionSet::LogAndApply(VersionEdit *edit, port::Mutex *mu, port::CondVar *cv, bool *wt,
...
unsigned int
VersionSet::RangeDifference(Slice a, Slice b)
{
...
uint64_t
VersionSet::GetOverlappingRangeBetweenFiles(FileMetaData *f1, FileMetaData *f2)
{
    ...
    if (ss < 0 && ls > 0)
    { // Second file's start is in the range of first file
        return RangeDifference(f2->smallest.user_key(), f1->largest.user_key());
    }
    if (ss > 0 && sl < 0)
    { // First file's start is in the range of second file
...
int
VersionSet::NumLevelFiles(unsigned level) const
{
    assert(level < config::kNumLevels);
...
int
VersionSet::NumGuards(unsigned level) const
{
    assert(level < config::kNumLevels);
...
VersionSet::GetGuardsAtLevel(unsigned level)
{
    assert(level < config::kNumLevels);
...
std::vector<GuardMetaData *>
VersionSet::GetCompleteGuardsAtLevel(unsigned level)
{
    assert(level < config::kNumLevels);
...
std::string
VersionSet::GetCurrentVersionState()
{
    if (current_ == NULL)
    {
...
int
VersionSet::NumGuardFiles(unsigned level) const
{
    assert(level < config::kNumLevels);
...
int
VersionSet::NumSentinelFiles(unsigned level) const
{
    assert(level < config::kNumLevels);
...
std::string
VersionSet::GuardDetailsAtLevel(unsigned level) const
{
    assert(level < config::kNumLevels);
...
std::string
VersionSet::SentinelDetailsAtLevel(unsigned level) const
{
    assert(level < config::kNumLevels);
...
const char *
VersionSet::LevelSummary(LevelSummaryStorage *scratch) const
{
...
int64_t
VersionSet::NumLevelBytes(unsigned level) const
{
    assert(level < config::kNumLevels);
...
void
VersionSet::GetRange(const std::vector<FileMetaData *> &inputs,
...
// REQUIRES: inputs is not empty
void
VersionSet::GetRange2(const std::vector<FileMetaData *> &inputs1,
...
Iterator *
VersionSet::MakeInputIteratorForGuardsInALevel(Compaction *c)
{
    ReadOptions options;
    ...
    for (int which = 0; which < 2; which++)
    {
        if (!c->inputs_[which].empty())
        {
            ...
            Iterator
                *guard_iterator = new Version::LevelGuardNumIterator(icmp_, guards, sentinel_files, files, 0, timer);
...
struct CmpByRange
{
    ...
    bool
    operator()(const FileMetaData *lhs, const FileMetaData *rhs)
    {
...
void
VersionSet::GetCompactionBoundaries(Version *v,
                                    unsigned level,
...
unsigned
VersionSet::NumUncompactedLevels()
{
...
bool
VersionSet::IsAllLevelsCompacted()
{
    for (int i = 0; i < config::kNumLevels; i++)
    {
        if (current_->compaction_scores_[i] >= 1.0)
...
unsigned
VersionSet::PickCompactionLevel(bool *locked, bool seek_driven, bool *force_compact) const
{
    ...
    *force_compact = false;
    ...
    if (level == config::kNumLevels && current_->compaction_scores_[config::kNumLevels - 1] >= 1.0 &&
        !locked[config::kNumLevels - 1])
    {
...
static bool
OldestFirst(FileMetaData *a, FileMetaData *b)
{
    return a->number < b->number;
}
                                    ...
                                    bool force_compact)
{
    ...
    // First, at the highest level (e.g,. Level 5) of FLSM, the sstables have to be rewritten during compaction;
    ...
    {
        // for the second-highest level (e.g,. Level 4), FLSM will rewrite an sstable into the same level if the
        // alternative is to merge into a large sstable in the highest level
        // (since we cannot attach new sstables in the last level if the guard is full).
    ...
    // Normally, current level and level below are involved in compaction and thus 2
    int num_input_levels_for_compaction = 2;
    if (horizontal_compaction)
    {
        // For horizontal_compaction, since we rewrite sstables into the same level and thus
        // only one level involved.
        num_input_levels_for_compaction = 1;
    }

    // Create a list of std::vector<GuardMetaData *> and the list size is 2
    ...
    for (int which = 0; which < num_input_levels_for_compaction; which++)
    {
        ...
        // Copy (concatenate) the v->complete_guards_[current_level] into (with)
        // complete_guards_copy[which]
    ...
    if (!horizontal_compaction)
    {
        // "The sstables in the guard are first (merge) sorted and then partitioned into new sstables
        // based on the guards of the next level" Here, complete_guards_used_in_bg_compaction is the "guards
        // of the next level"
    ...
    {
        // During horizontal_compaction, the "guards of the next level" is the complete_guards in the current level
        complete_guards_used_in_bg_compaction->insert(complete_guards_used_in_bg_compaction->begin(),
    ...
    for (int which = 0; which < num_input_levels_for_compaction; which++)
    {
        ...
        if (!add_sentinel_files && which == 0 &&
            (force_compact || v->sentinel_compaction_scores_[current_level] >= 1.0))
        {
            add_sentinel_files = true;
        ...
        for (size_t i = 0; i < complete_guards.size(); i++)
        {
            ...
            {
                int compare =
                    icmp_.user_comparator()->Compare(guards[guard_index_iter]->guard_key.user_key(), guard_key);
                if (compare == 0)
                {
            ...
            if (guard_index == -1)
            { // If guard is not found for this complete guard
                continue;
            }
            ...
            for (unsigned j = 0; j < g->files.size(); j++)
            {
                ...
                if ((i < complete_guards.size() -
                    1              // If it is not the last guard, checking for smallest and largest to fit in the range
                    && (icmp_.user_comparator()->Compare(file_smallest, guard_key) < 0
                        || icmp_.user_comparator()->Compare(file_largest, next_guard_key) >= 0))
                    || (i == complete_guards.size() -
            ...
            if (!guard_added && which == 0 &&
                (force_compact || v->guard_compaction_scores_[current_level][guard_index] >= 1.0))
            {
                guards_to_add_to_compaction.push_back(g);
...
bool
Compaction::CrossesBoundary(const ParsedInternalKey &old_key,
...
bool
Compaction::IsTrivialMove() const
{
    return num_input_files(1) == 0;
}

void
Compaction::AddInputDeletions(VersionEdit *ed)
{
    for (int which = 0; which < 2; which++)
    {
